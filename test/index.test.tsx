...
const DummyValidations: FormValidation<FormValues> = {
    ...
    nested: {
        data: {
            onChange: (value) => (value?.includes('secret') ? 'remove all secrets' : undefined),
...
const Input: React.FC<{ field: FormField<string> }> = ({ field }) => {
    const { onChange, onBlur, value, error, dirty, required, name } = Morfi.useField(field);
    const _onChange = useCallback((e: ChangeEvent<HTMLInputElement>) => onChange(e.target.value), [onChange]);
    return (
        <div>
            <input
                className={[required && 'required', dirty && 'dirty'].filter(Boolean).join(' ')}
                name={name}
                onChange={_onChange}
                onBlur={onBlur}
                value={value}
...
const OptionalInput: React.FC<{ field: FormField<string | undefined> }> = ({ field }) => {
    const { onChange, onBlur, value, error, dirty, required, name } = Morfi.useField(field);
...
describe('Morfi', () => {
    ...
    it('handles the form correctly', async () => {
        // when
        const ref: MutableRefObject<FormRef<FormValues> | null> = React.createRef<FormRef<any>>();
        const { container, rerender, unmount } = render(<DummyForm givenRef={ref} validation={DummyValidations} />);
        ...
        ref.current!.updateInitialData((data) => ({ ...data, name: 'Test Name Not Dirty' }));
        ...
        expect(container.querySelector('.error')!.textContent).toBe('No whitespace in displayName allowed');
        ...
        onSubmitFailed.wasCalledWith(
            Spy.COMPARE((err) => Morfi.isValidationError(err)),
            {
                ...
                isDirty: true,
                isSubmitting: false,
                values: { age: 100, displayName: 'Test-DisplayName', name: 'Test Name', nested: { data: '' } },
            }
        );
        ...
        expect(container.querySelector('.error')!.textContent).toBe('remove all secrets');
        fireEvent.change(inputs[3], { target: { value: '' } });
        expect(container.querySelector('.error')).toBe(null);
        fireEvent.submit(form);
        expect(container.querySelector('.error')!.textContent).toBe('Nested data required');
        onSubmit.wasNotCalled();
        onSubmitFinished.wasNotCalled();
        onSubmitFailed.wasNotCalled(); // will be called in the next tick to avoid automatic update batching
        await act(global.nextTick);
        onSubmitFailed.wasCalledWith(
            Spy.COMPARE((err) => Morfi.isValidationError(err)),
            {
                dirty: { age: true, displayName: true, name: true, 'nested.data': false },
                errors: { 'nested.data': 'Nested data required' },
                hasErrors: true,
                isDirty: true,
                isSubmitting: false,
                values: { age: 99, displayName: 'Test-DisplayName', name: 'Test Name', nested: { data: '' } },
            }
        );
        // fix the nested data
        ...
        onSubmitFailed.wasCalledWith(
            ...
            {
                dirty: { age: true, displayName: true, name: true, 'nested.data': true },
    ...
    it('handles async form validations', async () => {
        ...
        // then
        expect(container.querySelector('.error')!.textContent).toBe('Could not find such short names');
        ...
        onSubmitFailed.wasCalledWith(Spy.COMPARE(Morfi.isValidationError), {
            dirty: { name: true },
            errors: { age: 'Age required', name: undefined, 'nested.data': 'Nested data required' },
            hasErrors: true,
            isDirty: true,
            isSubmitting: false,
    ...
    it('Morfi.useClearErrors: Allows to clear errors from within the form tree', async () => {
        ...
        const DummyInner: React.FC<{ fields: FormFields<(typeof initialData)['values']> }> = ({ fields }) => {
            ...
            }, [clearErrors, fieldFoo.value, fields]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    it('Morfi.clearErrors: removes errors from MorfiData', () => {
        ...
        // test cases
        expect(Morfi.clearErrors(data, fields)).toEqual({ ...data, errors: {}, hasErrors: false });
